...
// ============================================================================
// === QuickChart QR helper (tanpa library tambahan) ===
const qrUrl = (text, size = 220) => 'https://quickchart.io/qr?text=' + encodeURIComponent(text) + '&size=' + size + '&margin=1&ecLevel=M';

// Ikon metode pembayaran (dipakai lintas komponen)
...
// Loader ExcelJS (export Excel berformat profesional)
        ...
        <div className="space-y-4 pb-20 overflow-y-auto h-full relative">
            ...
            {localStorage.getItem('biz_mode') === 'fnb' && (
              <div className="fixed bottom-24 inset-x-0 z-40 flex justify-center px-4 pointer-events-none">
                  <button onClick={() => { localStorage.getItem('scanner_mode') === 'camera' ? setLiveScanner({show:true, mode:'validation'}) : triggerAlert("Gunakan Scanner Fisik Anda untuk scan QR Customer.", "success") }} className="pointer-events-auto animate-slide-up bg-indigo-600 hover:bg-indigo-500 text-white rounded-full pl-3 pr-5 py-3 shadow-[0_10px_25px_rgba(79,70,229,0.4)] flex items-center gap-3 font-bold text-sm transition-transform active:scale-95 border-2 border-white/20">
...
const ReportTab = ({ licenseInfo, triggerAlert, activeTab }) => {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      titleCell.value = (prof.name || 'CostLab POS').toUpperCase() + '  —  LAPORAN PENJUALAN';
      ...
      headers.forEach((h, i) => {
        ...
        cc.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4F46E5' } };
        cc.alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
        cc.border = { top: { style: 'thin', color: { argb: 'FF312E81' } }, bottom: { style: 'thin', color: { argb: 'FF312E81' } }, left: { style: 'thin', color: { argb: 'FFC7D2FE' } }, right: { style: 'thin', color: { argb: 'FFC7D2FE' } } };
      });
      headerRow.height = 24;
      ...
      stats.list.forEach((tx, idx) => {
        ...
        const band = idx % 2 === 0 ? 'FFFFFFFF' : 'FFF1F5F9';
        row.eachCell((cc, col) => {
          cc.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: band } };
          cc.border = { bottom: { style: 'hair', color: { argb: 'FFE2E8F0' } } };
      ...
      totalRow.eachCell((cc, col) => {
        cc.font = { bold: true, color: { argb: 'FF0F172A' }, size: 11 };
        cc.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFEEF2FF' } };
        cc.border = { top: { style: 'double', color: { argb: 'FF4F46E5' } } };
                    ...
                    <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 transition-all">
                        ...
                        {tableMode && (
                            <div className="pt-3 border-t border-slate-200 dark:border-slate-700 animate-in fade-in">
                                ...
                                <div className="space-y-3">
                                    <button onClick={() => { const cnt = parseInt(tableCount); if(!cnt || cnt < 1) { triggerAlert('Isi jumlah meja dulu (minimal 1).', 'error'); return; } setShowTableQR(true); triggerAlert('Berhasil generate ' + cnt + ' QR meja!', 'success'); }} className="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/30 active:scale-95 transition"><QrCode className="w-4 h-4"/> Generate {tableCount || 0} QR Meja</button>
                                    {showTableQR && (
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-1">
                                            {Array.from({ length: Math.min(parseInt(tableCount) || 0, 200) }, (_, k) => k + 1).map(n => {
                                                const link = window.location.origin + '/?meja=' + n;
                                                return (
                                                    <div key={n} className="bg-white border border-slate-200 rounded-2xl p-3 flex flex-col items-center shadow-sm">
                                                        <img alt={'QR Meja ' + n} src={qrUrl(link, 220)} className="w-full aspect-square rounded-lg" />
                                                        <p className="font-black text-sm text-slate-700 mt-2">Meja {n}</p>
                                                        <a href={qrUrl(link, 700)} target="_blank" rel="noopener noreferrer" className="mt-1 text-[10px] font-bold text-indigo-600 flex items-center gap-1"><Download className="w-3 h-3"/> Buka / Unduh</a>
                                                    </div>
                                                );
                                            })}
                                        </div>
                                    )}
                                    <p className="text-[10px] text-slate-400 text-center leading-relaxed">QR otomatis ter-generate. Tempel di tiap meja; pelanggan scan untuk self-order langsung dari HP mereka.</p>
                                </div>
                            </div>
                        )}
                    </div>
