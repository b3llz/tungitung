...
import {
  Package, Zap, Plus, Trash2, Save, FolderOpen, RotateCcw,
  TrendingUp, Edit3, Image as ImageIcon, Layers, FileSpreadsheet,
  Printer, Crown, Briefcase, Receipt, Shield, CategoryIcon
} from './welp-icons.jsx';
import {
...
const SmartPlannerModal = ({ materials, production, onClose }) => {
  ...
  return (
    <Modal open onClose={onClose} title="Smart Production Planner" sub="Fitur Pro" width="max-w-lg">
      ...
      <Button variant="secondary" onClick={() => window.print()} icon={Printer} className="w-full py-3">Cetak Rencana Belanja</Button>
    </Modal>
  );
};
    ...
    <div className="w-full max-w-6xl mx-auto lg:grid lg:grid-cols-[1fr_400px] lg:gap-6 xl:gap-8 items-start">
      ...
      <div className="space-y-4 pb-32 lg:pb-8 min-w-0">
        ...
        <Card flush className="overflow-hidden">
          <div className="p-4 flex gap-4 items-center">
            <div className="w-20 h-20 shrink-0 bg-paper dark:bg-white/[.03] rounded-2xl border-2 border-dashed border-line dark:border-line-dark flex items-center justify-center relative group cursor-pointer hover:border-flame-400 transition-all overflow-hidden">
              {product.image ? <img src={product.image} className="w-full h-full object-cover" /> : <ImageIcon className="w-7 h-7 text-ink-faint/50" />}
        ...
        <div className="flex items-center justify-between px-1">
          ...
          {calcMode === 'detail' && <Badge tone="green"><Package className="w-3 h-3" /> {materials.length} bahan</Badge>}
        </div>
        ...
        {calcMode === 'detail' ? (
          ...
          <Card title="Mode Hitung Cepat" icon={Zap} help="Isi total uang belanja 1x produksi, lalu bagi dengan jumlah produk jadi.">
        ...
        <Card flush>
          <div className="flex justify-between items-center p-4 cursor-pointer" onClick={() => setShowFixed(!showFixed)}>
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition ${showFixed ? 'bg-flame-50 dark:bg-flame-900/40 text-flame-700 dark:text-apricot' : 'bg-paper dark:bg-white/5 text-ink-faint'}`}><Briefcase className="w-4 h-4" /></div>
              <div>
      ...
      <div className="lg:sticky lg:top-6 space-y-4 pb-32 lg:pb-8">
        ...
        {/* --- Tangga harga / tier --- */}
        <Card title="Saran Harga Jual" icon={Receipt} flush>
        ...
        {/* --- Cek kompetitor --- */}
        <Card title="Cek Kompetitor" icon={Shield} flush>
          <div className="p-4">
        ...
        <div className="hidden lg:grid grid-cols-4 gap-2">
          ...
          <Button variant="secondary" onClick={() => setShowLoad(true)} icon={FolderOpen} className="col-span-1">Load</Button>
          <Button onClick={save} icon={Save} className="col-span-2">Simpan Data</Button>
          <Button variant="secondary" onClick={() => isPro(licenseInfo) ? setShowPlanner(true) : triggerAlert("Fitur PRO Only", "error")} icon={Layers}
            className={`col-span-2 ${!isPro(licenseInfo) && 'opacity-60'}`}>
            Smart Planner {!isPro(licenseInfo) && <Crown className="w-3 h-3 text-gold" />}
          </Button>
          <Button onClick={handleExportExcel} disabled={isExporting} icon={FileSpreadsheet} variant="bright" className="col-span-2">
            {isExporting ? 'Mengekspor...' : 'Export (.xlsx)'}
          </Button>
        </div>
        <div className="lg:hidden grid grid-cols-2 gap-2">
          <Button variant="secondary" onClick={() => isPro(licenseInfo) ? setShowPlanner(true) : triggerAlert("Fitur PRO Only", "error")} icon={Layers} className={!isPro(licenseInfo) && 'opacity-60'}>
            Smart Planner {!isPro(licenseInfo) && '(PRO)'}
          </Button>
          <Button onClick={handleExportExcel} disabled={isExporting} icon={FileSpreadsheet} variant="bright">
