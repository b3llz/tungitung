// ============================================================
// TAB OPERASIONAL v5 WELP — Riwayat Transaksi, Kas Keluar,
// Diskon/Pajak, Manajemen Karyawan. Logika 100% dipertahankan.
// ============================================================
import React, { useState, useEffect } from 'react';
import {
  Search, Uang, Qris, KasKeluar, Save, Trash2, Diskon, Coins,
  Karyawan, RefreshCw, Lock, Unlock, Store, TrendingUp, Layers,
  MonitorSmartphone, DatabaseBackup, Riwayat
} from './welp-icons.jsx';
...
export const DiscountTab = ({ triggerAlert }) => {
  ...
  const rows = [
    ...
    { key: 'service', title: 'Biaya Layanan (Service Charge)', desc: 'Biaya layanan tambahan (Maksimal 100%).', icon: Coins, placeholder: 'Contoh: 5' },
...
/* ================= MANAJEMEN KARYAWAN ================= */
export const EmployeeTab = ({ triggerAlert }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto w-full pb-24 space-y-5">
      <PageTitle title="Manajemen User" sub="Hak Akses & PIN Karyawan" />

      <Card title="Tambah Karyawan" icon={Karyawan}>
        <div className="space-y-4">
          <div>
            <label className="kicker block mb-1.5 ml-0.5">Nama Karyawan</label>
            <input className="field-lg" placeholder="Nama Karyawan" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
          </div>
          <Select label="Role Akses" value={form.role} options={['kasir', 'admin']} onChange={v => setForm({ ...form, role: v })} />
          <div>
            <label className="kicker block mb-1.5 ml-0.5">PIN Akses (6 Digit)</label>
            <div className="flex gap-2">
              <div className="flex-1 h-[46px] bg-paper dark:bg-white/5 border border-line dark:border-line-dark rounded-xl px-4 flex items-center justify-center tracking-[0.5em] font-extrabold text-ink dark:text-ink-inv text-sm select-all">
                {form.pin ? form.pin : '••••••'}
              </div>
              <Button onClick={generatePin} variant="secondary" className="px-4 h-[46px]"><RefreshCw className="w-4 h-4" /></Button>
            </div>
          </div>
      ...
      <div className="space-y-2.5">
        <h3 className="font-extrabold text-[13px] text-ink dark:text-ink-inv px-1">Daftar Akses ({employees.length})</h3>
        {employees.length === 0 ? <EmptyState mascot="menyapa" title="Belum ada karyawan terdaftar" desc="Tambahkan akses PIN untuk kasir/admin agar mereka bisa login dengan aman tanpa berbagi PIN owner." /> : employees.map(e => (
          <div key={e.id} className="card p-4 flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-flame-50 dark:bg-flame-900/40 text-flame-700 dark:text-apricot flex items-center justify-center rounded-full font-extrabold">{e.name[0]}</div>
              <div><p className="font-extrabold text-[13px] text-ink dark:text-ink-inv">{e.name}</p><p className="text-[10px] text-ink-faint font-bold uppercase tracking-widest">{e.role}</p></div>
            </div>
            <div className="flex items-center gap-2.5">
              <button
                onClick={() => setVisiblePinId(visiblePinId === e.id ? null : e.id)}
                className="text-xs font-mono font-extrabold bg-paper dark:bg-white/5 px-2.5 py-1.5 rounded-lg flex items-center gap-1.5 hover:bg-flame-50 dark:hover:bg-flame-900/40 transition text-ink-soft dark:text-ink-inv/70"
                title="Klik untuk tampilkan/sembunyikan PIN"
              >
                <Lock className={`w-3 h-3 ${visiblePinId === e.id ? 'hidden' : ''}`} />
                {visiblePinId === e.id ? <Unlock className="w-3 h-3 text-gold" /> : null}
                PIN: {visiblePinId === e.id ? e.pin : '••••••'}
              </button>
              <button onClick={() => deleteEmployee(e.id)} className="text-ink-faint hover:text-brick p-1.5 hover:bg-brick-soft dark:hover:bg-brick/10 rounded-lg transition"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
