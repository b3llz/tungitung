...
import {
  Laporan, TrendingUp, Wallet, Keranjang, Download, Award,
  AlertTriangle, Rocket, Uang, Qris, Stok, HppCalc, Riwayat
} from './welp-icons.jsx';
  ...
  return (
    <div className="max-w-5xl mx-auto w-full pb-24 space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Kpi icon={TrendingUp} label={`Omzet (${filter})`} value={formatIDR(stats.rev)} tone="hero" note={`${stats.count} transaksi tercatat`} />
        <Kpi icon={Wallet} label="Laba Kotor" value={formatIDR(profitStats.grossProfit)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {/* MARGIN HEALTH */}
        <Card title="Produk Margin Terendah" icon={AlertTriangle}>
        ...
        {/* STOCK VELOCITY */}
        <Card title="Perkiraan Stok Habis" icon={Rocket}>
        ...
        <Card title="Riwayat Transaksi" icon={Riwayat} action={
          <Button onClick={handleDownloadReport} icon={Download} variant="secondary" className="py-1.5 h-8 text-[10px]">{isDownloading ? 'Mengekspor…' : 'Export Excel'}</Button>
        }>
