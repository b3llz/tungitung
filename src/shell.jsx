// ============================================================
// SHELL v6 — Navigasi WELP: sidebar charcoal netral dengan
// BrandLockup (WELP + by JUSTru GROUP), pill flame sebagai
// state aktif, maskot asli menyapa di kaki sidebar.
// MOBILE: satu-satunya trigger menu ada di BottomNav (slot
// "Menu"). Header mobile TIDAK punya hamburger lagi sehingga
// tidak ada menu dobel. Model RBAC (NAV_GROUPS) dipertahankan.
// ============================================================
import React from 'react';
import {
  Kasir, HppCalc, Riwayat, KasKeluar, Diskon, Stok, Opname, InOut,
  StokRiwayat, Supplier, Laporan, Karyawan, Outlet, Toko, Bayar,
...
export const NAV_GROUPS = [
  ...
  { key: 'biz', labelKey: 'business', items: [
    ...
    { id: 'employee', labelKey: 'employee', icon: Karyawan, roles: ['owner'] },
    { id: 'outlet', labelKey: 'outlet', icon: Outlet, roles: ['owner'] },
...
export const Sidebar = ({ role, active, setActive, licenseInfo, dark, toggleDark, onLogout, lowStockCount, editing }) => (
  <aside className={`hidden lg:block fixed inset-y-0 left-0 w-[260px] p-3 z-40 transition-all duration-300 ${editing ? 'opacity-40 blur-sm pointer-events-none' : ''}`}>
    <div className="h-full bg-chrome-deep rounded-[1.5rem] flex flex-col overflow-hidden shadow-dock border border-chrome-edge relative">
      {/* brand: WELP primer + by JUSTru GROUP sekunder */}
      <div className="p-4 pb-3.5 border-b border-chrome-edge/70">
        <div className="flex items-start justify-between gap-2">
          <BrandLockup theme="dark" size="md" withTagline={false} />
        ...
        <p className="text-ink-inv/45 text-[10px] font-bold mt-2 truncate">
          {licenseInfo?.tenant ? `${licenseInfo.tenant} · ${role}` : 'Kasir & HPP dalam satu genggaman'}
        </p>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto custom-scrollbar px-2.5 py-3 space-y-4">
        {NAV_GROUPS.map(group => {
          ...
          return (
            <div key={group.key}>
              <p className="text-[8.5px] font-extrabold text-ink-inv/35 uppercase tracking-[0.2em] mb-1.5 px-3">{t(group.labelKey)}</p>
                  ...
                  return (
                    <button key={item.id} onClick={() => setActive(item.id)}
                      className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[12.5px] transition press ${isActive
                        ...
                        : 'text-ink-inv/60 font-bold hover:bg-chrome-panel hover:text-ink-inv'}`}>
      ...
      <div className="px-3 pt-2 border-t border-chrome-edge/70">
        <div className="flex items-center gap-2.5 py-2 px-1">
          <div className="w-9 h-9 rounded-xl bg-chrome-panel border border-chrome-edge flex items-center justify-center overflow-hidden shrink-0">
            <Mascot pose="menyapa" className="w-8 h-8 object-contain translate-y-0.5" alt="" />
          </div>
          <div className="min-w-0">
            <p className="text-ink-inv/85 text-[11px] font-extrabold truncate leading-none">{licenseInfo?.tenant || 'Welp'}</p>
            <p className="text-ink-inv/40 text-[9px] font-bold mt-1 truncate">{role === 'owner' ? 'Owner' : role === 'admin' ? 'Admin' : 'Kasir'}</p>
          </div>
        </div>
        <div className="pb-2.5 space-y-0.5">
          <button onClick={toggleDark} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[12.5px] font-bold text-ink-inv/60 hover:bg-chrome-panel hover:text-ink-inv transition">
...
export const BottomNav = ({ active, setActive, onMenu, lowStockCount }) => {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-chrome-deep/95 backdrop-blur-md border-t border-chrome-edge pb-safe">
        ...
        {items.map((it, idx) => {
          ...
          return (
            <button key={idx} onClick={() => it.id ? setActive(it.id) : onMenu()} aria-label={it.label}
              className={`relative flex flex-col items-center justify-center gap-1 transition press ${isActive ? 'text-apricot' : 'text-ink-inv/45 hover:text-ink-inv/80'}`}>
...
export const MenuSheet = ({ open, onClose, role, active, setActive, licenseInfo, onLogout, dark, toggleDark }) => {
  ...
  const en = getLang() === 'en';
  return (
    <div className="lg:hidden fixed inset-0 z-[200] flex justify-end bg-chrome-deep/70 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="w-[85%] max-w-sm bg-chrome-deep h-full shadow-pop flex flex-col animate-slide-up border-l border-chrome-edge" onClick={e => e.stopPropagation()}>
        <div className="p-5 border-b border-chrome-edge flex justify-between items-start">
          <div>
            <BrandLockup theme="dark" size="sm" withTagline={false} endorsement align="center" />
            <p className="text-[10px] font-extrabold text-ink-inv/40 uppercase tracking-[0.18em] mt-2.5">{t('access')}: {licenseInfo?.currentUserRole || '-'}</p>
          </div>
          <button onClick={onClose} aria-label="Tutup menu" className="w-9 h-9 rounded-full bg-white/5 text-ink-inv/60 hover:text-ink-inv flex items-center justify-center transition"><X className="w-4 h-4" /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3.5 space-y-5">
          {NAV_GROUPS.map(group => {
            ...
            return (
              <div key={group.key}>
                <p className="text-[8.5px] font-extrabold text-ink-inv/35 uppercase tracking-[0.2em] mb-2 px-2">{t(group.labelKey)}</p>
                <div className="space-y-0.5">
                  {items.map(item => {
                    const isActive = active === item.id;
                    const Icon = item.icon;
                    return (
                      <button key={item.id} onClick={() => { setActive(item.id); onClose(); }}
                        className={`w-full flex items-center gap-3 p-3 rounded-xl transition press ${isActive
                          ? 'bg-flame-500 text-white font-extrabold'
                          : 'text-ink-inv/60 font-bold hover:bg-chrome-panel hover:text-ink-inv'}`}>
        ...
        <div className="p-3.5 border-t border-chrome-edge space-y-0.5">
          <button onClick={toggleDark} className="w-full flex items-center gap-3 p-3 rounded-xl text-[13px] font-bold text-ink-inv/60 hover:bg-chrome-panel hover:text-ink-inv transition">
          ...
          <p className="text-center text-[9px] font-extrabold text-ink-inv/25 uppercase tracking-[0.2em] pt-2">WELP v8 · Fresh Ink</p>
        </div>
      </div>
    </div>
  );
