  ...
  return (
    <div className="max-w-6xl mx-auto w-full pb-24 space-y-5 animate-rise">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MetricCard label="Omzet Hari Ini" value={formatIDR(snapshot.omzetToday)} sub={`${snapshot.todayTxs.length} transaksi`} icon={Omzet} />
        <MetricCard label="Transaksi" value={String(snapshot.todayTxs.length)} sub="tercatat hari ini" icon={Riwayat} />
        {canSeeCash && <MetricCard label="Kas Keluar" value={formatIDR(snapshot.expenseToday)} sub="pengeluaran hari ini" icon={KasKeluar} />}
        {isOwnerLike && (
          snapshot.profitKnown
            ? <MetricCard label="Laba Kotor" value={formatIDR(snapshot.grossProfit)} sub="dari HPP saat penjualan" icon={LabaKotor} tone="flame" />
